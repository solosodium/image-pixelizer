...
describe('Color (color.js)', () => {
	...
	describe('RGBA', () => {
		...
		it('test color difference function', () => {
			...
			assert(RGBA.difference(rgba1, rgba2) > 0);
		});
		...
		it('test to LABAlpha', () => {
			...
			assert(isClose(labaplga.l, 36.03, 0.01));
			assert(isClose(labaplga.a, 70.42, 0.01));
			assert(isClose(labaplga.b, -69.91, 0.01));
			assert(isClose(labaplga.alpha, 255, 0.01));
		});
	});

});

function isClose(num1, num2, delta) {
